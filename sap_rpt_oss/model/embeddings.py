...
class CellEmbeddings(nn.Module):
    ...
    def __init__(
        ...
        if regression_type == "l2":
            ...
            self.number_embeddings = nn.Embedding(
        ...
        if regression_type == "l2":
            ...
            self.target_embedding_layer_reg = nn.Embedding(
                Tokenizer.QUANTILE_DIMENSION, config.hidden_size
            )
        self.target_embedding_layer_classif = nn.Embedding(
            Tokenizer.QUANTILE_DIMENSION, config.hidden_size
        )
    ...
    def forward(self, input_dict: Dict, is_regression: bool):
        ...
        if self.regression_type == "l2":
            numbers_normalized = (
                input_dict["number_normalized"]
                .unsqueeze(-1)
                .type(self.number_embeddings.weight.dtype)
            )
            number_embeds = self.number_embeddings(numbers_normalized)
            number_embeds = torch.where(
                numbers_normalized <= -99,
                torch.zeros_like(number_embeds),
                number_embeds,
            )
        else:
            number_perc_floor = input_dict["number_percentile_floor"]
            number_embeds = torch.zeros(
                (num_rows, num_cols, self.hidden_size),
            ...
            next_perc = torch.minimum(
                number_perc_floor[mask] + 1,
                torch.tensor(Tokenizer.QUANTILE_DIMENSION - 1),
            )
            number_embeds_plus_one = self.number_embeddings(next_perc)
            delta = (
                input_dict["number_percentile_delta"][mask]
                .type(number_embeds.dtype)
                .unsqueeze(-1)
            )
            number_embeds[mask] = (
                number_embeds[mask] * (1 - delta) + number_embeds_plus_one * delta
            )

        date_embeds = self.date_embeddings(
            input_dict["date_year_month_day_weekday"]
        )  # (rows, cols, embed_dim)

        unsqueezed = input_dict["column_embeddings"].unsqueeze(
            0
        )  # (1, cols, embed_dim)
        column_embeds = self.column_remapping(
            unsqueezed.type(self.column_remapping.weight.dtype)
        )
        ...
        content_embeds = self.content_remapping(
            input_dict["text_embeddings"].type(self.content_remapping.weight.dtype)
        )
        if self.is_target_content_mapping:
            content_embeds[:, -1] = (
        ...
        if is_classification and self.is_target_content_mapping:
            ...
            target_text_embeddings = target_text_embeddings.type(
                self.target_content_remapping.weight.dtype
            )
            ...
            if self.regression_type == "l2":
                target_values_reg = (
                    input_dict["target"]
                    .unsqueeze(-1)
                    .type(self.target_embedding_layer_reg.weight.dtype)
                )
                target_embeds_reg = self.target_embedding_layer_reg(target_values_reg)
                target_embeds_reg = torch.where(
                    target_values_reg <= -99,
                    torch.zeros_like(target_embeds_reg),
                    target_embeds_reg,
                )
                ...
                delta = (
                    input_dict["target_delta"]
                    .type(target_embeds_reg.dtype)
                    .unsqueeze(-1)
                )
                target_embeds_reg = (
